/**
 * @file PesquisaBinaria.c
 * @author 784778 - Wallace Freitas Oliveira (https://github.com/Olivwallace)
 * @brief TP02Q08 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 20-03-2023
 */
...
int main(int argc, char** argv) {
	...
	FILE* arq = fopen("784778_binaria.txt", "w");
	...
	if (inicializaLista(lista, 80) != BAD_ALLOC) {
		...
		while (caminho[0] != 'F' && caminho[1] != 'I' && caminho[2] != 'M') { //Percorre lista realizando buscas de entradas; 

			if (buscaBinaria(lista, caminho, retornoAux)) {
				printf("SIM\n");
			}else {
				printf("NAO\n");
			}

			numComparacoes += retornoAux[1]; //Incrementa soma de comparacoes a cada busca
			lerBufferStdin(caminho, BUFFER_64);
		}
	...
	if (arq != NULL) { //Escreve em arquivo se aberto
		fprintf(arq, "%s\t%gs\t%d", MATRICULA, runTime, numComparacoes);
		fclose(arq);
	}
