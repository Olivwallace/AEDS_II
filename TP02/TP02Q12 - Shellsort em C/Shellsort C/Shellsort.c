 ...
 * @brief TP02Q12 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 31-03-2023
 */
...
error shellsort(listaSeq_t* lista, int *retornoaux);
...
int main(int argc, char** argv) {
	...
	FILE* arq = fopen("784778_shellsort.txt", "w");
...
/// <summary>
/// Ordena lista de personagens por meio de ShellSort
/// </summary>
...
error shellsort(listaSeq_t* lista, int* retornoaux) {
	...
	if (lista != NULL && lista->listaSeq != NULL && retornoaux != NULL) {
		...
		for (h = 0; h < numItens; h = (3 * h) + 1);

		do {
			h /= 3;

			for (uint32_t i = h; i < numItens; i += h) {
				uint32_t j = i;
				dado_t aux = personagens[j];

				while (j > h - 1 && personagens[j - h]->getPeso(personagens[j - h]) > aux->getPeso(aux)) {
					retornoaux[0]++; retornoaux[1]++;
					personagens[j] = personagens[j - h];
					j -= h;
				}
