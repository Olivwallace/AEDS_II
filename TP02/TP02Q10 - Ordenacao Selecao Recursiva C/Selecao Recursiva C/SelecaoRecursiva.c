/**
 * @file SelecaoRecursiva.c
 * @author 784778 - Wallace Freitas Oliveira (https://github.com/Olivwallace)
 * @brief TP02Q10 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 24-03-2023
 */
...
error selecaoSort(listaSeq_t* lista, int* retornoAux);
error selecaoSortRec(dado_t* lista, uint32_t posIni, uint32_t posFim, int * retornoAux);
error swap(dado_t *lista, uint32_t posIdeal, uint32_t posAtual);
...
int main(int argc, char** argv) {
	...
	FILE* arq = fopen("784778_selecaoRecursiva.txt", "w");
	...
	if (inicializaLista(lista, 50) != BAD_ALLOC) {
		...
		selecaoSort(lista, countComp_Mov);
		imprimeLista(lista);
...
/// <summary>
/// Funcao intermediaria de odernacao por selecao
/// </summary>
...
error selecaoSort(listaSeq_t* lista, int *retornoAux) {
	...
	if (lista != NULL && lista->listaSeq != NULL) {
		status = selecaoSortRec(lista->listaSeq, 0, lista->numItens, retornoAux); //Chama funcao principal recursiva
	}
...
/// <summary>
/// Ordena lista por meio de selecao recursiva
/// </summary>
/// <param name="lista">Lista a ser ordenada</param>
/// <param name="posIni">Posicao inicial</param>
/// <param name="posFim">Posicao final</param>
/// <param name="retornoAux">Vetor auxiliar de retorno</param>
/// <returns>Status de erro</returns>
error selecaoSortRec(dado_t* lista, uint32_t posIni, uint32_t posFim, int* retornoAux) {
	error status = NULL_EXCEPTION;
	...
	if (lista != NULL) {
		string atual, comparado;
		uint32_t min = posIni;

		for (uint32_t i = (min + 1); i < posFim; i++) {

			atual = lista[min]->getNome(lista[min]); //Recupera nome do menor atual
			comparado = lista[i]->getNome(lista[i]); //Recupera nome da pos comparada

			if (strcmp(atual, comparado) > 0) { //Compara valores
				min = i;
			}
		...
		if (min != posIni) { //Se valor minimo e posicao inicial forem diferentes: troca
			retornoAux[1]++; //Acrescentar numero de moviementacoes realizadas
			swap(lista, posIni, min);
		}

		if (posIni + 1 < posFim) { //Chama recursivamente para ordenar restante da lista.
			status = selecaoSortRec(lista, posIni + 1, posFim, retornoAux);
		}else {
			status = SUCESSO;
		}
	}
...
/// <summary>
/// Realiza troca de valores entre posicoes ideal e atual
/// </summary>
/// <param name="lista">Lista ordenada</param>
/// <param name="posIdeal">Posica destino</param>
/// <param name="posAtual">Posicao origem</param>
/// <returns>Status de erro</returns>
error swap(dado_t* lista, uint32_t posIdeal, uint32_t posAtual) {
	error status = NULL_EXCEPTION;

	if (lista != NULL) {
		dado_t tmp = lista[posIdeal];
		lista[posIdeal] = lista[posAtual];
		lista[posAtual] = tmp;
		status = SUCESSO;
	}
