 ...
 * @brief TP02Q16 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 31-03-2023
 */
...
error bubbleSort(listaSeq_t* lista, int* retornoaux);
...
int main(int argc, char** argv) {
	...
	FILE* arq = fopen("784778_bubbleSort.txt", "w");
	...
	if (inicializaLista(lista, 50) != BAD_ALLOC) {
		...
		bubbleSort(lista, countComp_Mov);
		imprimeLista(lista);
...
/// <summary>
/// Ordena lista de personagens por meio de BubbleSort
/// </summary>
...
error bubbleSort(listaSeq_t* lista, int* retornoaux) {
	...
	if (lista != NULL && lista->listaSeq != NULL && retornoaux != NULL) {
		...
		uint32_t h, numItens = lista->numItens;
		dado_t* personagens = lista->listaSeq;

		for (uint32_t i = 0; i < numItens; i++) {
			for (uint32_t j = (numItens - 1); j > i; j--) {
				string anoAtual = personagens[j]->getAnoNascimento(personagens[j]);
				string anoComp = personagens[j - 1]->getAnoNascimento(personagens[j - 1]);
				
				retornoaux[0]++;
				if (strcmp(anoAtual, anoComp) < 0) {
					dado_t tmp = personagens[j];
