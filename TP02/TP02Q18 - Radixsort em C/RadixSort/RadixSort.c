 ...
 * @brief TP02Q18 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 01-04-2023
 */
...
void radixSort(listaSeq_t* lista, int* retornoaux);
int alturaMax(dado_t* personagens, int numItens);
...
int main(int argc, char** argv) {
	...
	FILE* arq = fopen("784778_radixsort.txt", "w");
...
void radixSort(listaSeq_t* lista, int* retornoaux) {
	if (lista != NULL && lista->listaSeq != NULL && retornoaux != NULL) {
		...
		int exp, j;
		listaSeq_t *temp = listaCria();
		dado_t* personagens = lista->listaSeq;
		int numItens = lista->numItens, max = alturaMax(personagens, numItens);
		inicializaLista(temp, lista->tamMax);

		for (exp = 1;  max / exp > 0; exp *= 10) {
	
			int count[10] = {0};

			if (temp != NULL && temp->listaSeq != NULL) {

				for (j = 0; j < numItens; j++) {
					int alt = personagens[j]->getAltura(personagens[j]);
					int pos = (alt / exp) % 10;
					count[pos]++;
				}


				for (j = 1; j < BUFFER_256; j++) {
					count[j] += count[j - 1];
				}


				for (j = numItens - 1; j >= 0; j--) {
					int alt = personagens[j]->getAltura(personagens[j]);
					int pos = (alt / exp) % 10;
					int posF = count[pos] - 1;
					insereFinal(temp, clona(personagens[j]));
					count[pos]--;
				}


				for (j = numItens - 1; j >= 0 ; j--) {
					personagens[j] = removeFinal(temp);
				}

			}
		}	

		for (uint32_t i = 0; i < temp->numItens; i++) destrutorPersonagem(temp->listaSeq[i]); //Desaloca personagens presentes na lista
		listaClear(temp);
	}
}
...
/// Obtem altura maxima
/// </summary>
/// <param name="personagens">Lista de personagens</param>
/// <param name="numItens">Numero de itens</param>
/// <returns>Altura maxima</returns>
int alturaMax(dado_t* personagens, int numItens) {
	int max = 0;

	if (personagens != NULL) {
		for (int i = 0; i < numItens; i++) {
			int tam = personagens[i]->getAltura(personagens[i]);
			if (tam > max) max = tam;
		}
	}
