/**
 * @file InsercaoParcial.c
 * @author 784778 - Wallace Freitas Oliveira (https://github.com/Olivwallace)
 * @brief TP02Q20 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 06-04-2023
 */
...
error imprimeLista(listaSeq_t* lista, int k);
void listaClear(listaSeq_t* lista);
boolean buscaBinaria(listaSeq_t* lista, string dado, int* retornoAux);
void heapSort(listaSeq_t* lista, int max, int* retornoAux);
void criarHeap(listaSeq_t* lista, int tam, int pos, int* retornoAux);
int compararAltura(dado_t atual, dado_t comparado);
...
int main(int argc, char** argv) {
	...
	FILE* arq = fopen("784778_heapsortparcial.txt", "w");
	...
	if (inicializaLista(lista, 50) != BAD_ALLOC) {
		...
		heapSort(lista, 10 ,countComp_Mov);
...
/// </summary>
/// <param name="lista">Objeto de lista de armazenamento</param>
/// <param name="k">Numero de itens a ser impresso</param>
/// <returns>Status de impressao (ERROR)</returns>
error imprimeLista(listaSeq_t* lista, int k) {
	error status = NULL_EXCEPTION;

	if (lista != NULL && lista->listaSeq != NULL) {
		int numIntens = (int)lista->numItens;
...
/// <summary>
/// Realiza comparacao de 
/// </summary>
/// <param name="atual"></param>
/// <param name="comparado"></param>
/// <returns></returns>
int compararAltura(dado_t atual, dado_t comparado) {
	int atualAltura = atual->getAltura(atual), compAltura = comparado->getAltura(comparado);
	int result = atualAltura - compAltura;
...
/// <summary>
///	Ordena lista por meio de heapsort parcial.
/// </summary>
/// <param name="lista">Lista a ser ordenada</param>
/// <param name="max">Maximo de itens ordenados</param>
/// <param name="retornoAux">Array de retorno auxiliar</param>
void heapSort(listaSeq_t* lista, int max, int* retornoAux) {
	if (lista != NULL && lista->listaSeq != NULL && retornoAux != NULL) {
		int numItens = (int)lista->numItens;
		dado_t* personagens = lista->listaSeq;
		
		for (int i = (max / 2) - 1; i >= 0; i--) {
			criarHeap(lista, max, i, retornoAux);
		}

		for (int i = max; i < numItens; i++) {
			if (compararAltura(personagens[1], personagens[i]) < 0) {
				dado_t temp = personagens[1];
				personagens[1] = personagens[i];
				personagens[i] = temp;

				retornoAux[1] += 3;

				criarHeap(lista, numItens, max, retornoAux);
			}
		...
		for (int i = numItens - 1; i > 0; i--) {
			dado_t temp = personagens[0];
			personagens[0] = personagens[i];
			personagens[i] = temp;

			retornoAux[1] += 3;

			criarHeap(lista, i, 0, retornoAux);
		}
...
/// <summary>
/// Funcao para criar heap recursivamente
/// </summary>
/// <param name="lista">Lista ordenada</param>
/// <param name="tam">Tamanho maximo</param>
/// <param name="pos">Posicao atual raiz</param>
void criarHeap(listaSeq_t* lista, int tam, int pos, int *retornoAux) {
	if (lista != NULL && lista->listaSeq != NULL) {
		dado_t* personagens = lista->listaSeq;
		int raiz = pos, esq = (2 * raiz) + 1, dir = (2 * raiz) + 2;

		if (esq < tam && compararAltura(personagens[esq], personagens[raiz]) > 0) raiz = esq;
		if (dir < tam && compararAltura(personagens[dir], personagens[raiz]) > 0) raiz = dir;
		
		retornoAux[0] += 2;

		if (raiz != pos) {
			dado_t temp = personagens[pos];
			personagens[pos] = personagens[raiz];
			personagens[raiz] = temp;
			
			retornoAux[1] += 3;

			criarHeap(lista, tam, raiz, retornoAux);
		}
