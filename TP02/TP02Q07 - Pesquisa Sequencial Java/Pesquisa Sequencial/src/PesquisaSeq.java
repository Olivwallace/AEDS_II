/**
 * @file PesquisaSeq.java
 * @author 784778 - Wallace Freitas Oliveira (https://github.com/Olivwallace)
 * @brief TP02Q07 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 24-03-2023
 */
...
public class PesquisaSeq {
    ...
    private static final File arq = new File(matricula + "_sequencial.txt");
    ...
    public static void main(String[] args){
        ...
        while (!in.contains("FIM")) { //Carrega lista com primeiras entradas
            try {
        ...
        while(!in.contains("FIM")){
            int[] retornoAux = new int[2];

            MyIO.println((listaPersonagens.buscaSequencial(retornoAux, in))?"SIM":"NAO");

            numComparacoes += retornoAux[1];
            in = MyIO.readLine().replace('é', '\u00e9');
        }

        long endTime = System.currentTimeMillis(); //Finaliza Temporizador
        long tempo = endTime - startTime;

        try{
            writer.write(matricula + "\t" + tempo / 1000.0 + "s\t" + numComparacoes);
