/**
 * @file CountingSortJava.java
 * @author 784778 - Wallace Freitas Oliveira (https://github.com/Olivwallace)
 * @brief TP02Q15 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 01-04-2023
 */
...
public class CountingSortJava {
    ...
    private static final File arq = new File(matricula + "_countingsort.txt");
...
class ListaSeq {
    ...
    /**
     * Ordena lista através de CountingSort
     */
    public void CountingSort(){
        setLogSort(new int[2]);
        int maiorPeso = (int)getMaiorPeso();
        int[] count = new int[(int)lista[maiorPeso].getPeso() + 1]; //Instancia vetor de contagem


        for(int i = 0; i < numItens; i++) count[(int)lista[i].getPeso()]++; //Conta elementos
        for(int i = 1; i < count.length; i++) count[i] += count[i - 1];     //Conclui contagem


        Personagem[] arrayOrdenado = new Personagem[lista.length];          //Vetor auxiliar de ordenacao
        for(int i = numItens - 1; i >= 0; i--){                             //Posiciona cada elemento na posicao ideal
            arrayOrdenado[--count[(int)lista[i].getPeso()]] = lista[i];     //do vetor auxiliar.
        }


        for(int i = 0; i < lista.length; i++){        //Transfere "imagem" de array ordenado
            logSort[1]++;                             //para array principal
            lista[i] = arrayOrdenado[i];
        }
    }

    /**
     * Obtem posicao do maior peso encontrado em Array
     * @return Posicao do personagem de maior peso
     */
    public int getMaiorPeso(){
        int maior = 0;

        for(int i = 1; i < numItens; i++){
            if(lista[i].getPeso() > lista[maior].getPeso()) maior = i;
            logSort[0]++;
        }
