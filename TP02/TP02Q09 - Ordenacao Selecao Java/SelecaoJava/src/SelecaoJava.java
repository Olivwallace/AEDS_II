/**
 * @file SelecaoJava.java
 * @author 784778 - Wallace Freitas Oliveira (https://github.com/Olivwallace)
 * @brief TP02Q09 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 24-03-2023
 */
...
public class SelecaoJava {
    ...
    private static final File arq = new File(matricula + "_selecao.txt");
...
class ListaSeq {
    ...
    /**
     * Ordena lista por meio de selecao
     */
    public void selecaoSort(){
        ...
        for(int i = 0; i < numItens; i++){
            ...
            for(int j = (i + 1); j < numItens; j++){
                atual = lista[menor].getNome();
                comparado = lista[j].getNome();

                if(atual.compareToIgnoreCase(comparado) > 0){
                    menor = j;
                }
    ...
    /**
     * Realiza troca de posicoes na lista
     * @param podDes Posicao de destino
     * @param posOri Posicao de origem
     */
    public void swap(int podDes, int posOri){
        if(podDes != posOri){
            Personagem tmp = lista[podDes];
            lista[podDes] = lista[posOri];
            lista[posOri] = tmp;
