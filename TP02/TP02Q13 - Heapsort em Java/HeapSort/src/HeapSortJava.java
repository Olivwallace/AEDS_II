...
public class HeapSortJava {
    ...
    private static final File arq = new File(matricula + "_heapsort.txt");
...
class ListaSeq {
    ...
    /**
     * Ordena lista através do heapSort
     */
    public void HeapSort(){
        ...
        //Extrai elementos do heap e realiza novo
        for(int i = tam - 1; i > 0; i--){
            Personagem temp = lista[0];
            lista[0] = lista[i];
            lista[i] = temp;
    ...
    /**
     * Aplica heap recursivamente na arvore.
     * @param tam Tamanho maximo
     * @param pos Posicao da raiz atual
     */
    private void aplicaHeap(int tam, int pos){
        int raiz = pos, esq = (2 * raiz) + 1, dir = (2 * raiz) + 2;
        logSort[0] += 2;

        //Avalia folha esquerda
        if(esq < tam && lista[esq].getAltura() >= lista[raiz].getAltura()){
            if(lista[esq].getAltura() > lista[raiz].getAltura() || lista[esq].getNome().compareTo(lista[raiz].getNome()) > 0){
                raiz = esq;
            }
        }

        //Avalia folha direita
        if(dir < tam && lista[dir].getAltura() >= lista[raiz].getAltura()){
           if(lista[dir].getAltura() > lista[raiz].getAltura() || lista[dir].getNome().compareTo(lista[raiz].getNome()) > 0){
               raiz = dir;
           }
        }

        //Se alguma folha for maior que a raiz original troca e chama recursivamente no galho.
        if(raiz != pos){
            Personagem temp = lista[pos];
            lista[pos] = lista[raiz];
            lista[raiz] = temp;
