...
void insercaoSortParcial(listaSeq_t* lista, int* retornoAux, int k);
int compararAnoNascimento(dado_t atual, dado_t comparado);
...
int main(int argc, char** argv) {
	...
	FILE* arq = fopen("784778_insercaoparcial.txt", "w");
	...
	if (inicializaLista(lista, 50) != BAD_ALLOC) {
		...
		insercaoSortParcial(lista, countComp_Mov, 10);
...
/// <returns></returns>
int compararAnoNascimento(dado_t atual, dado_t comparado) {
	int result = strcmp(atual->getAnoNascimento(atual), comparado->getAnoNascimento(comparado));
...
/// <summary>
/// Realiza ordenacao por insercao parcial.
/// </summary>
/// <param name="lista">Endereco de lista</param>
/// <param name="retornoAux">Vetor de retorno auxiliar</param>
/// <param name="k">Numero de itens ordenados desejado</param> 
void insercaoSortParcial(listaSeq_t* lista, int* retornoAux, int k) {
	if (lista != NULL && lista->listaSeq != NULL) {
		dado_t* personagens = lista->listaSeq;

		for (int i = 1; i < lista->numItens; i++) {
			int j = (i < k) ? i - 1 : k - 1;
			dado_t aux = personagens[i];

			retornoAux[0]++;
			while (j >= 0 && compararAnoNascimento(aux, personagens[j]) < 0) {
				retornoAux[0]++;
				retornoAux[1]++;
