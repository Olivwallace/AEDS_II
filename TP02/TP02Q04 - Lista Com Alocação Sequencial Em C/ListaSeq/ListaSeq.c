...
void operaLista(listaSeq_t* lista, int qtdOperacoes);
...
error insereInicio(listaSeq_t* lista, dado_t dado);
error inserePosicao(listaSeq_t* lista, dado_t dado, uint32_t pos);
error insereFinal(listaSeq_t* lista, dado_t dado);

dado_t removeInicio(listaSeq_t* lista);
dado_t removePosicao(listaSeq_t* lista, int pos);
...
int main(int argc, char** argv) {
	...
	if (inicializaLista(&lista, 80) != BAD_ALLOC) {
		...
		operaLista(&lista, atoi(numOp));
		
		for (int i = 0; i < lista.numItens; i++) destrutorPersonagem(lista.listaSeq[i]); //Desaloca personagens presentes na lista
...
/// <summary>
/// Metodo para operar lista
/// </summary>
/// <param name="lista">Lista a ser operada</param>
...
void operaLista(listaSeq_t* lista, int qtdOperacoes) {
	if (lista != NULL) {
		...
		for (int i = 0; i < qtdOperacoes; i++) {
			...
			op = split(lido, ' '); //Armazena operacao e dado a ser operado em lista

			personagem = novoPersonagem();

			if (personagem != NULL && op != NULL) {

				if (op[0][0] == 'I') { //Se operacao for insercao verifica qual o tipo de insercao a ser realizada.
					switch (op[0][1]){
						case 'I':
							personagem->ler(personagem,op[1]);
							insereInicio(lista, personagem);
							break;
						case 'F':
							personagem->ler(personagem, op[1]);
							insereFinal(lista, personagem);
							break;
						case '*':
							personagem->ler(personagem, op[2]);
							inserePosicao(lista, personagem, atoi(op[1]));
							break;
					}
				}else { //Se nao insercao eh remocao, verifica o tipo de remocao a ser execultada.
					switch (op[0][1]) {
						case 'I':
							personagem = removeInicio(lista);
							printf("(R) %s\n", personagem->getNome(personagem));
							break;
						case 'F':
							personagem = removeFinal(lista);
							printf("(R) %s\n", personagem->getNome(personagem));
							break;
						case '*':
							personagem = removePosicao(lista, atoi(op[1]));
							printf("(R) %s\n", personagem->getNome(personagem));
							break;
					}
		...
		imprimeLista(lista); //Imprime conteudo restante em lista;
	}
}
...
/// <summary>
/// Realiza insercao em inicio de lista.
/// </summary>
/// <param name="lista">Objeto de lista armazenamento</param>
...
error insereInicio(listaSeq_t* lista, dado_t dado) {
	...
	if (lista != NULL && lista->listaSeq != NULL) {
		...
		if (numItens < tamanho) {
			...
			for (int i = numItens; 0 < i; i--) {
				listaDados[i] = listaDados[i - 1];
			}
...
/// <summary>
/// Realiza insercao em posicao passada como parametro
/// </summary>
/// <param name="lista">Objeto de lista armazenamento</param>
/// <param name="dado">Dado a ser inserido</param>
/// <param name="pos">Posicao de insercao</param>
/// <returns>Status de Insercao (ERROR)</returns>
error inserePosicao(listaSeq_t* lista, dado_t dado, uint32_t pos) {
	error status = NULL_EXCEPTION;
	...
	if (lista != NULL && lista->listaSeq != NULL) {
		...
		if (numItens < tamanho && pos > 0 && pos < tamanho) {
			for (uint32_t i = numItens; pos < i; i--) {
				listaDados[i] = listaDados[i - 1];
			}
...
/// <summary>
/// Remove dado presente no inicio da lista.
/// </summary>
/// <param name="lista">Objeto de lista de armazenamento</param>
...
dado_t removeInicio(listaSeq_t* lista) {
	...
	if (lista != NULL && lista->listaSeq != NULL && lista->numItens != 0) {
...
/// <summary>
/// Remove dado presente em posicao especificada
/// </summary>
/// <param name="lista">Objeto de lista de armazenamento</param>
/// <param name="pos">Posicao desejada de remocao</param>
/// <returns>Dado contido na posica recebida</returns>
dado_t removePosicao(listaSeq_t* lista, int pos) {
	dado_t dado = NULL;
	...
	if (lista != NULL && lista->listaSeq != NULL) {
		...
		for (uint32_t i = pos; i < tamanho; i++) {
			listaDados[i] = listaDados[i + 1];
		}
	}
