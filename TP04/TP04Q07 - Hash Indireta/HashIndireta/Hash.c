 ...
 * @brief TP04Q07 - Algoritmo e Estruturas de Dados II (PUC-Minas 1°/2023)
 * @date 17-06-2023
 */
...
//---------------- Definicao da HashTable

typedef struct itemTabela_s {
	Personagem_t* dado;
	struct itemTabela_s* prox;
} itemTabela_t;
...
int buscarAltura(string nome, noAVL_t* i, int* comp);
void liberarAVL(noAVL_t* no);

void inicializarLista(lista_t* lista);
void inicializarHash(hash_t* hash, int tam);
int funcHash(int altura, int tamTab);
void insereHash(hash_t* hash, Personagem_t* dado, int* comp);
boolean buscaHash(hash_t* hash, string nome, int* comp);
boolean insereListaHash(lista_t* lista, Personagem_t* dado);
void liberaHash(hash_t* hash);
...
int main(int argc, char** argv) {
	...
	FILE* arq = fopen("784778_hashIndireta.txt", "w");
	...
	while (caminho[0] != 'F' && caminho[1] != 'I' && caminho[2] != 'M') { //Se nao atingiu FIM, continua processo.
		...
		insereHash(&hash, personagem, &numOp);
	...
	while (caminho[0] != 'F' && caminho[1] != 'I' && caminho[2] != 'M') { //Se nao atingiu FIM, continua processo.
		printf("%s ", caminho);
		printf(((buscaHash(&hash, caminho, &numOp)) ? "SIM\n" : "NÃO\n"));
		lerBufferStdin(caminho, BUFFER_64); //Ler nova entrada
	}
...
void inicializarHash(hash_t* hash, int tam) {
	if (hash != NULL) {
		...
		hash->tam = tam;
		hash->avl = novaArvore();
		hash->tabela = malloc(tam * sizeof(lista_t));
...
void insereHash(hash_t* hash, Personagem_t* dado, int *comp) {
	if (hash != NULL && dado != NULL) {
		int pos = funcHash(dado->altura, hash->tam);

		if(insereListaHash(&hash->tabela[pos], dado))
			hash->avl->raiz = inserir(dado, hash->avl->raiz, comp);
	}
}
...
boolean buscaHash(hash_t* hash, string nome, int *comp) {
	...
	if (hash != NULL && nome != NULL) {
		int alt = buscarAltura(nome, hash->avl->raiz, comp);
		int pos;

		if (alt >= 0) {
			pos = funcHash(alt, hash->tam);

			itemTabela_t *atual = hash->tabela[pos].prim;
			
			while (!existe && atual != NULL){
				existe = strcmp(atual->dado->nome, nome) == 0;
				atual = atual->prox;
				comp++;
			}
...
boolean insereListaHash(lista_t* lista, Personagem_t* dado) {
	...
	if (lista != NULL && dado != NULL) {
		...
		itemTabela_t* novo = malloc(sizeof(itemTabela_t));
...
int buscarAltura(string nome, noAVL_t* i, int* comp) {
	...
	if (i != NULL && comp != NULL) {
		...
		if (strcmp(i->personagem->nome, nome) == 0) {
			comp++;
			alt = i->personagem->altura;
		}
		else if (strcmp(i->personagem->nome, nome) < 0)
		{
			comp += 2;
			alt = buscarAltura(nome, i->dir, comp);
		}
		else if (strcmp(i->personagem->nome, nome) > 0) {
			comp += 3;
			alt = buscarAltura(nome, i->esq, comp);
		}
